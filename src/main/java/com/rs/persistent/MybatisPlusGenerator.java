...
public class MybatisPlusGenerator {
	...
	private static ConfigGenerator setupMysql(ConfigGenerator cg){
		...
		cg.setDbUrl("jdbc:mysql://192.168.10.64:3306/apihtml?useUnicode=true&characterEncoding=UTF8");
		return cg;
	}
	...
	private static ConfigGenerator getConfigGenerator(){
        ...
        cg.setEntityPackage("com.rs.persistent");//entity 实体包路径
        cg.setMapperPackage("com.rs.persistent.mapper");//mapper 映射文件路径
        cg.setServicePackage("com.rs.service");//service 层路径
        cg.setXmlPackage("com.rs.persistent.mapper");//xml层路径（可以不写）
        cg.setServiceImplPackage("com.rs.service.impl");//serviceimpl层路径（可以不写）
